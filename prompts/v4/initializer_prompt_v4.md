...
## YOUR ROLE — INITIALIZER AGENT (Session 1, v4: Export Options + Episode Library)
...
This project builds **PodcastBrain v4** — a Streamlit web application that downloads
audio from YouTube, transcribes locally with Whisper, runs three Claude AI analyses,
provides interactive Q&A, and adds: export to Markdown/PDF/SRT/TXT/ZIP formats and
a searchable Episode Library backed by SQLite FTS5 full-text search.
...
Based on `app_spec.txt`, create a file named `feature_list.json` with exactly **27**
...
- Priority: foundational first (app loads → input → pipeline → viewer → exports → library → style)
...
**Feature areas to cover (27 total):**

1. App loads at port 8501 with sidebar navigation (Process New, Episode Library, Batch Queue stub)
...
9. Claude full analysis runs: summary, quotes, action items
...
13. Chapters tab displays chapter list with timestamps
14. Transcript tab displays text with timestamps and search
...
20. TXT export downloads a plain text file with transcript content
21. ZIP export downloads an archive containing multiple files
22. Episode Library page displays processed episodes in a 3-column card grid
23. FTS5 search by keyword finds episodes containing that word in the transcript
24. Date range filter in library narrows results correctly
25. Clicking an episode card in the library opens its 4-tab viewer without reprocessing
26. Style: episode cards in library use grid layout (not list)
27. Style: no layout overflow at standard 1280px window width
...
podcastbrain/
  **init**.py
  app.py         — Streamlit entry point: sidebar nav, 3 pages (Process New, Library, Batch stub)
...
**v4 DB schema — 4 ORM tables + FTS5 virtual table:**
...
def init_db():
    ...
    IMPORTANT: FTS5 virtual tables cannot be created via SQLAlchemy ORM.
    They must be created via raw SQL using conn.execute(text(...)).
    Call Base.metadata.create_all() first, then create FTS5 and trigger.
    """
...
- Use `CREATE VIRTUAL TABLE IF NOT EXISTS` — safe to call on every startup
- Use `CREATE TRIGGER IF NOT EXISTS` — safe to call on every startup
- The trigger auto-populates `transcripts_fts` on every INSERT into `transcripts`
- FTS5 search uses parameterized queries ONLY — never string interpolation (SQL injection risk)
...
7. **app.py** — 3-page sidebar + Episode Library with FTS5 search + export buttons in viewer
...
3. **Confirm feature_list.json** is valid JSON with exactly **27** entries, all `"passes": false`
...
**Remember:** The FTS5 setup is the most critical and tricky part.
Get `init_db()` right before anything else — future agents depend on it.
